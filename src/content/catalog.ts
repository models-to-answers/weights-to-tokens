...
export const animations = [
  ...
  ["animation.model-factory.preference-trainer", "Train a preference signal", ["Comparison 1", "Comparison 2", "Comparison 3"]],
  ["animation.model-factory.adaptation-lab", "Inspect a LoRA update", ["Full fine-tuning", "Freeze the base", "Insert low-rank pair", "Reconstruct update", "Merge and serve"]],
  ["animation.model-factory.fine-tune-methods", "Compare fine-tuning methods", ["Full fine-tuning", "LoRA", "QLoRA", "Adapters", "Prefix tuning"]],
  ["animation.model-factory.artifact-packaging", "Inspect a model release spectrum", ["Closed API", "Open weights", "Open-ish", "Reproducible open"]],
  ["animation.inference-system.request-arrival", "Prepare a request", ["Authenticate", "Route", "Admit", "Batch"]],
  ["animation.inference-system.compute-platform", "Peel the compute platform stack", ["Bare metal", "Virtual machine", "Kubernetes", "Kubernetes on VMs"]],
  ["animation.inference-system.model-locality", "Move an artifact to a warm GPU", ["Artifact registry", "Node cache", "Pinned host memory", "GPU HBM", "Warm worker"]],
  ["animation.inference-system.model-loading", "Ready an inference worker", ["Locate artifact", "Start workers", "Reserve GPU memory", "Load weights", "Warm up", "Declare ready"]],
  ["animation.inference-system.prefill-decode", "Follow single-GPU inference", ["Tokenize", "Batch", "Prefill", "Create KV cache", "Decode one token", "Append and repeat"]],
  ["animation.inference-system.multi-gpu", "Compare multi-GPU strategies", ["One GPU", "Independent replicas", "Split one model", "Coordinate", "Choose deliberately"]],
  ["animation.inside-gpu.throughput-silicon", "Compare CPU and GPU silicon budgets", ["Serial latency", "Parallel throughput", "Transformer inference"]],
  ["animation.inside-gpu.zoom-anatomy", "Zoom into a GPU", ["GPU package", "High-bandwidth memory", "Streaming multiprocessor", "Warp scheduler", "Execution units", "On-chip memory"]],
  ["animation.inside-gpu.kernel-launch", "Launch one kernel", ["Host call", "Driver command", "Grid created", "Block admitted", "Warps formed", "Instructions issue", "Memory completes", "Kernel retires"]],
  ["animation.inside-gpu.divergence", "Simulate warp divergence", ["Uniform path", "Split branch", "Masked path A", "Masked path B", "Reconverge"]],
  ...
  ["animation.inside-gpu.roofline", "Diagnose a GPU kernel with a roofline", ["Measure work", "Measure bytes", "Compute intensity", "Locate bottleneck", "Choose optimization"]],
  ["animation.replay.one-prompt", "Replay one prompt end to end", ["Weights are learned", "The artifact is stored", "The model becomes ready", "A prompt is admitted", "Text becomes token IDs", "Prefill processes the prompt", "The view enters the GPU", "A kernel is launched", "Warps execute instructions", "Memory supplies operands", "Decode predicts a token", "The token is streamed"]],
