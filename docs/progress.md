...
| 3 | Shared deterministic animation runtime and retained/new rich interactions | 23 chapter interactions verified |
...
- Replaced the starter preview and optional database example with a frontend-only
  academy shell.
- Added a 13-chapter typed catalog, 26 globally unique questions, 24 animation
...
- Added direct `/learn/:slug` chapter routes and `/replay`, with reload-safe
  navigation.
- Added a coherent 13-chapter learner-facing journey, Q&A feedback,
...
- Restored “Build one yourself” as the scored fifth Model Factory chapter. Its
  six-stage design carries a genuinely distinct assistant, domain, or
  on-device preset through architecture, pretraining, alignment, release, and
  an inference-ready build sheet. Expert exposes the assumptions and controls
  without duplicating the animation or its saved state.
